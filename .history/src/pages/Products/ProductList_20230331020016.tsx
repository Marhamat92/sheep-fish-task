...
function ProductList({}: Props) {
  ...
  const { products, isLoading, isError, message, isSuccess, categories } =
  ...
  //use inputHandler to handle input change and search product by name

  const [productsRows, setProductsRows] = React.useState<any>([]);
  ...
  const inputHandler = (e: any) => {
    ...
    if (value.length > 0) {
      ...
      setProductsRows(filteredProducts);
    } else {
      dispatch(getAllProducts() as any);
    }
  };

  //fix products.map is not a function
  const rows = productsRows.length > 0 ? productsRows : products || [];
    ...
    <div>
      <div className='container mt-6'>
        <div className='flex justify-between items-center'>
          <div>
            <SearchField inputHandler={inputHandler} categories={categories} />
          </div>
          <div>
            <button
